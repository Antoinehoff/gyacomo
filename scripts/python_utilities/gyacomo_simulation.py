...
class GyacomoSimulation:
    ...
    def __init__(self, run_dir, jobnum=0, executable=None):
        ...
        self.params = self._init_params_structure()
    ...
    def load_params_from_file(self, filename):
        ...
        for namelist, values in parsed.items():
            if namelist == 'SPECIES':
                ...
                self.params['SPECIES'] = values
            elif namelist in self.params:
                self.params[namelist].update(values)
            else:
                self.params[namelist] = values
    ...
    def write_params_file(self, filename=None):
        ...
        with open(filename, 'w') as f:
            ...
            for namelist in ['BASIC', 'GRID', 'GEOMETRY', 'DIAGNOSTICS', 
                ...
                if namelist not in self.params:
                    continue
                
                if namelist == 'SPECIES':
                    # Write each species separately
                    for species in self.params['SPECIES']:
                        f.write(f"&{namelist}\n")
                    ...
                    f.write(f"&{namelist}\n")
                    for key, value in self.params[namelist].items():
                        f.write(f"  {key:15} = {self._format_value(value):15}")
                        f.write(f" ! {self._get_param_comment(namelist, key)}\n")
                    f.write("/\n")
    ...
    def run(self, nproc=8, blocking=True, verbose=True):
        """
        ...
        # Determine MPI decomposition (Nx * Ny * 1)
        Nx_procs = self.params['GRID']['Nx']
        Ny_procs = nproc // Nx_procs
        
        # Build command
        cmd = [
            'mpirun', '-np', str(nproc),
            str(self.executable),
            str(Nx_procs), str(Ny_procs), '1'
        ]
        
        if verbose:
            print(f"Running: {' '.join(cmd)} < params.in")
    ...
    def check_status(self):
        ...
        if status['output_exists']:
            try:
                # Load time data
                t0d = loader.load_h5path(str(self.output_file), 'data/var0d/time')
                status['current_time'] = float(t0d[-1])
                status['max_time'] = self.params['BASIC']['tmax']
    ...
    def load_params(self):
        ...
        return loader.load_params(str(self.output_file))
    ...
    def load_grids(self):
        ...
        if not self.output_file.exists():
            raise FileNotFoundError(f"Output file not found: {self.output_file}")
        return loader.load_grids(str(self.output_file))
    ...
    def load_time_trace(self, varname):
        ...
        if not self.output_file.exists():
            raise FileNotFoundError(f"Output file not found: {self.output_file}")
        return loader.load_data_0D(str(self.output_file), varname)
    ...
    def load_field_3d(self, varname, time=None):
        ...
        if time is None:
            ...
            t3d = loader.load_h5path(str(self.output_file), 'data/var3d/time')
            time = t3d[-1]
        
        return loader.load_data_3D_frame(str(self.output_file), varname, time)
    ...
    def field_to_realspace(self, field_kxkyz, iz=-1):
        ...
        """
        return tools.zkxky_to_xy_const_z(field_kxkyz, iz)
    ...
    def compute_growth_rate(self, field_name='phi', kx_val=0.0, ky_index=None, 
        ...
        # Find kx index
        ikx = tools.closest_index(kx, kx_val)
        ...
        for it in time_indices:
            _, field, _ = loader.load_data_3D_frame(
        ...
        for iky in ky_indices:
            omega_t, time_out = tools.compute_omega_t(t3d, values[:, iky])
